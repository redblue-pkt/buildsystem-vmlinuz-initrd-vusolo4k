	...
	struct device_attribute *attr, char *buf)
{
	...
	return dump_core_regs(__BMIPS_GET_CBR(), regs, ARRAY_SIZE(regs), buf);
}
#endif
	...
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		...
		"0x%08lx 0x%08lx 0x%08lx 0x%08lx 0x%08lx\n",
		bmips_read_zscm_reg(0x00), bmips_read_zscm_reg(0x10),
		bmips_read_zscm_reg(0x20), bmips_read_zscm_reg(0x30),
		bmips_read_zscm_reg(0x40), bmips_read_zscm_reg(0x50),
		bmips_read_zscm_reg(0x60), bmips_read_zscm_reg(0x70),
		bmips_read_zscm_reg(0x80), bmips_read_zscm_reg(0x90));
}
#endif
