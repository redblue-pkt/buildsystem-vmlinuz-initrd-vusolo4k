...
static struct private_data *driver_data;
static struct cpufreq_frequency_table *global_freq_table;
...
static ssize_t show_brcmstb_safe_freq(struct cpufreq_policy *policy, char *buf)
{
	...
	for (i = 0; global_freq_table[i].frequency != CPUFREQ_TABLE_END; i++) {
		if (!(global_freq_table[i].driver_data & BRCMSTB_TBL_SAFE_MODE))
			safe_freq = global_freq_table[i].frequency;
	}
